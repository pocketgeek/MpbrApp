...
- `AmmoCategory` enum — `RIFLE`, `RIMFIRE`, `PISTOL`; defaults to `RIFLE` so only non-rifle presets need an explicit tag
...
**Adding ammo presets** — append to `Ballistics.PRESETS`. G7 model is specified as the 7th constructor argument (`DragModel.G7`); G1 is the default. BC passed to the constructor must match the drag model (do not mix G1 BCs with G7 model or vice versa). Always set `category = AmmoCategory.RIMFIRE` or `category = AmmoCategory.PISTOL` for non-rifle rounds; rifle is the default and needs no explicit tag. Keep presets grouped by category in the list — the dropdown inserts section headers by detecting category changes in order.
