...
- *`SIG_MRAD_MILLING`* / *`SIG_MOA_MILLING`*: SIG Tango-MSR Milling 2.0 dedicated styles — fine continuous grid (ticks every `minorSpacing`, numbered every `majorSpacing` out to `postStart`), short thin top post (`2 * majorSpacing` above center, no thick posts), full elevation/windage mesh below center out to `postStart` (major grid lines every `majorSpacing`), thick outer H posts beyond `±postStart` and thick bottom V post beyond `postStart`. Not shared with any other preset — see below.
...
**Reticle style policy (as of v1.93)** — going forward, new reticle presets get their own dedicated `ReticleStyle` enum value and drawing branch rather than being mapped onto an existing shared style, so each preset's on-screen artwork can be an exact match to its manufacturer diagram instead of an approximation. `SIG_MRAD_MILLING`/`SIG_MOA_MILLING` are the first pair built this way — see them for the pattern (fine tick helper function + full mesh grid, all parameterized off the same `ReticlePreset` fields as the shared styles). The older shared styles (`BDC`, `MRAD_TREE`, `MOA_TREE`, `CIRCLE_DOT`, etc.) still cover most existing presets and are being migrated to dedicated styles incrementally, preset by preset, verified against the manufacturer's actual reticle diagram or photo each time — not in bulk.
